package com.purchase.sls.common.widget.Banner;
...
import com.purchase.sls.common.widget.Banner.transform.DepthPageTransformer;
import com.purchase.sls.common.widget.Banner.transform.RotateDownPageTransformer;
import com.purchase.sls.common.widget.Banner.transform.ZoomOutPageTransformer;
...
//        banner.setBackgroundResource(resId);
...
//                    Glide.with(context).load(url).centerCrop().crossFade().into(iv);
                    Glide.with(context).load(url).fitCenter()/*.transform(new CornersTransform(context,10))*/
                            .crossFade().into(iv);
                else
//                    Glide.with(context).load(url).centerCrop().crossFade().placeholder(defaultImage).into(iv);
                    Glide.with(context).load(url).fitCenter()/*.transform(new CornersTransform(context,10))*/
                            .placeholder(defaultImage).crossFade().into(iv);
            }
        }
        setData();
    }
...
//            iv.setCornerRadiusDimen(R.dimen.marker_photo_corner);
            ...
            if(imageListener!=null){
                ...
                if (defaultImage!=-1)
                    Glide.with(context).load(url).fitCenter().crossFade().into(iv);
//                    Glide.with(context).load(url).fitCenter().transform(new CornersTransform(context,10))
//                            .crossFade().into(iv);
                else
                    Glide.with(context).load(url).fitCenter().crossFade().placeholder(defaultImage).into(iv);
//                    Glide.with(context).load(url).fitCenter().transform(new CornersTransform(context,10))
//                            .placeholder(defaultImage).crossFade().into(iv);
            }
        }
