...
public class SharePresenter implements EnergyContract.SharePresenter {
    private RestApiService restApiService;
    private List<Disposable> mDisposableList = new ArrayList<>();
    private EnergyContract.ShareView shareView;

    @Inject
    public SharePresenter(RestApiService restApiService, EnergyContract.ShareView shareView) {
    ...
    public void share() {
        ...
        Disposable disposable = restApiService.share(tokenRequest)
                .flatMap(new RxRemoteDataParse<String>())
                .compose(new RxSchedulerTransformer<String>())
                .subscribe(new Consumer<String>() {
                    @Override
                    public void accept(String string) throws Exception {
                        shareView.success(string);
                    }
                }, new Consumer<Throwable>() {
                    @Override
                    public void accept(Throwable throwable) throws Exception {
                        shareView.showError(throwable);
                    }
                });
